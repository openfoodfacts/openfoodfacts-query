...
async def test_create_product_scans():
    async with database_connection() as connection:
      ...
      code1 = random_code();
      code2 = random_code();
      await connection.execute("insert into product (code) values ($1),($2)", code1, code2)

      await import_scans({
        code1: {
          # this one shouldn't be included in the totals
          (OLDEST_YEAR - 1): ScanCounts(scans_n=3,unique_scans_n=2,unique_scans_n_by_country={"uk": 2,"world": 2}),
          OLDEST_YEAR: ScanCounts(scans_n=7,unique_scans_n=3,unique_scans_n_by_country={"uk": 3,"world": 3}),
          CURRENT_YEAR: ScanCounts(scans_n=10,unique_scans_n=7,unique_scans_n_by_country={"uk": 2, "fr": 5, "world": 7}),
        },
        code2: {
          CURRENT_YEAR: ScanCounts(scans_n=11,unique_scans_n=8,unique_scans_n_by_country={"ru": 1, "fr": 4, "world": 5}),
        },
      })
      result = await connection.fetch("select * from product_scans_by_country where product_id = (select id from product where code = $1)", code1)
      assert len(result) == 7

      product_countries = await connection.fetch("""select * from product_country pc
