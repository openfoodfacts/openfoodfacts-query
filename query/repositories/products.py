async def create_table(connection):
    await connection.execute(
        'create table product (id uuid not null, data jsonb null, name text null, code text null, last_modified timestamp null, creator text null, owners_tags text null, last_update_id uuid null, obsolete boolean null default false, constraint product_pkey primary key (id));',
    )
    await connection.execute(
...
async def change_primary_key(connection):
    await connection.execute(
