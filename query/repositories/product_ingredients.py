async def create_table(connection):
    await connection.execute(
        'create table product_ingredient (product_id int not null, sequence text not null, parent_product_id int null, parent_sequence text null, ingredient_text text null, id text null, ciqual_food_code text null, percent_min double precision null, percent text null, percent_max double precision null, percent_estimate double precision null, data json null, obsolete boolean null default false, constraint product_ingredient_pkey primary key (product_id, sequence));',
    )
    await connection.execute(
