...
async def test_aggregate_events_by_count_and_distinct_products():
    async with database_connection() as connection:
        ...
        await create_events(connection, [
            sample_event({"code": code1, "user_id": "user1", "action": "updated", "rev": 1}),
            sample_event({"code": code1, "user_id": "user1", "action": "updated", "rev": 2}),
            sample_event({"code": code1, "user_id": "user1", "action": "updated", "rev": 3}),
            sample_event({"code": code2, "user_id": "user1", "action": "updated", "rev": 1}),
        ])

    # use viewer user
    async with readonly_connection() as viewer:
        results = await viewer.fetch("select * from product_updates_by_owner where owner_tag = $1", owner1)
...
async def test_update_existing_aggregate_counts():
    async with database_connection() as connection:
        ...
        action1 = random_code()
        await create_events(connection, [
            sample_event({"code": code1, "user_id": "user1", "action": action1, "rev": 1}),
        ])

        # add another message
        await create_events(connection, [
            sample_event({"code": code1, "user_id": "user1", "action": action1, "rev": 2}),
        ])

    # use viewer user
    async with readonly_connection() as viewer:
        results = await viewer.fetch("select * from product_updates_by_owner where update_type = $1", action1)
        assert len(results) == 1
