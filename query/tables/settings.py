...
async def apply_pre_migration_message_id():
    ...
    async with get_transaction() as transaction:
        setting = await transaction.fetch(
            "UPDATE settings SET last_message_id = pre_migration_message_id, pre_migration_message_id = NULL WHERE pre_migration_message_id IS NOT NULL RETURNING last_message_id"
        )
        if len(setting) == 1:
